    ...
    <div className="min-h-screen bg-transparent relative flex flex-col items-center justify-center p-4 font-sans text-white overflow-hidden">
      ...
      <div className="w-full max-w-sm relative z-10 animate-fade-in-up">
        <div className="mb-8 text-center">
          ...
          <p className="text-textMuted text-sm font-medium tracking-wide">Welcome back. Let's trace some signals.</p>
        </div>
        ...
        <div className="glass-panel p-8 rounded-2xl">
          <form onSubmit={submit} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full bg-brand text-background font-bold text-sm py-3.5 rounded-lg hover:bg-brand-hover hover:box-glow transition-all disabled:opacity-50 disabled:cursor-not-allowed mt-4 shadow-[0_0_15px_rgba(0,255,136,0.15)] flex justify-center items-center gap-2"
            >
              {loading ? (
                <>
                ...
                <><span className="font-mono text-background/70">$</span> login</>
              )}
            </button>
          </form>
        </div>
        ...
        <p className="text-center text-textMuted text-sm mt-8 font-medium">
          No account?{' '}
          <Link to="/register" className="text-brand hover:text-brand-hover hover:underline underline-offset-4 transition-colors">
            Register here
          </Link>
        </p>
      </div>
